...
public class EmptyWrapper<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean isEmpty() {
        if (mInnerAdapter instanceof HeaderAndFooterWrapper) {
            headerCount = ((HeaderAndFooterWrapper) mInnerAdapter).getHeadersCount();
            return (mEmptyView != null || mEmptyLayoutId != 0) && (mInnerAdapter.getItemCount() - ((HeaderAndFooterWrapper) mInnerAdapter).getHeadersCount()) == 0;
        }
    ...
    private boolean hasHeader() {
        if (mInnerAdapter instanceof HeaderAndFooterWrapper) {
            return ((HeaderAndFooterWrapper) mInnerAdapter).getHeadersCount() > 0;
        } else {
            return false;
        }
    }
    ...
    public int getItemViewType(int position) {
        if (isEmpty()) {
            if (hasHeader()) {
                if (position <= getItemCount()-headerCount ) {
    ...
    public int getItemCount() {
        if (isEmpty()) {
            if (hasHeader()) {
                return ((HeaderAndFooterWrapper) mInnerAdapter).getHeadersCount()+1;
            }
            return 1;
        }
